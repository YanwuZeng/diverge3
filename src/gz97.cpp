...
reconstruct(int treeE,
  ...
  int op = Progress::progress();
  for(i = 0; i < seqLength; i++) {
#ifdef DEBUG_DIVERGE
    printf("%d / %d\n", i+1, seqLength);
#endif
    Progress::set_progress(op + int(100.0/seqLength*(i+1)+.5));
...
gammaDistanceNew1(const BMatrix2D &seqN, const DVector &length,
 ...
 // return (iZero+0.0)/(seqLength+0.0);
}
